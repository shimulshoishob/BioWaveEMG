...
import or require main.py - it has its own wired (USB/serial) and wireless
...
Requires: PyQt5, numpy, joblib, pyserial, rf_features.py (feature extractor
matching whatever model you trained), and pyautogui for actually moving the
cursor.
"""
...
# Feature extractor used by whatever model you trained in the main app.
try:
    from rf_features import extract_window_features
    HAS_RF_FEATURES = True
except ImportError:
    extract_window_features = None
...
class MouseControllerApp(QMainWindow):
    ...
    def apply_dark_theme(self):
        self.setStyleSheet("""
            QMainWindow, QWidget { background-color: #132440; color: #E8EEF0; font-family: 'Segoe UI', Arial; font-size: 13px; }
            ...
            QTabWidget::pane { border: 1px solid #3B9797; border-radius: 4px; }
            QTabBar::tab { background: #16476A; color: #E8EEF0; padding: 6px 14px; }
            QTabBar::tab:selected { background: #3B9797; color: white; }
